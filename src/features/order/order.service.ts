...
export class OrderService extends BaseService {
  ...
  async restaurantActiveOrders(restaurantId: string): Promise<OrderDto[]> {
    const dbQuery:any[] = await this.firebase.getDataWithWhereQuery(
  ...
  async updateOrderState(params:OrderDto):Promise<boolean|HttpException>{
    try {
      if(params.orderState=="Teslim Edildi"||params.orderState=="İptal Edildi"){
        //TODO: Update restaurant revenue data
        ...
        await this.firebase.updateData(FirebaseColumns.ORDERS,params.orderId,OrderDto.toJson(params));
      }
      this.gateway.orderUpdate(params,params.orderId);
      return true;
    } catch (error) {
      return new HttpException('Bir sorun oluştu, tekrar deneyiniz.',HttpStatus.BAD_REQUEST,);
    }
  }

  private async takeLogExpiredOrder(order:OrderDto):Promise<boolean|HttpException>{
    try {
      await this.logDatabase.db.collection(order.restaurantId).doc("LOGS").collection(FirebaseColumns.ORDERS).doc(order.orderId).set(OrderDto.toJson(order));
      return true;
    } catch (error) {
      return new HttpException('Bir sorun oluştu, tekrar deneyiniz.',HttpStatus.BAD_REQUEST,);
    }
